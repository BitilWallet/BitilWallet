...
class TodayAPI {
  ...
  static func fetchPrice(currency: String, completion: @escaping ((Dictionary<String, Any>?, Error?) -> Void)) {
    guard let url = URL(string: "https://api.bitilwallet.com/currentprice.json") else {return}
  ...
  static func getUserPreferredCurrency() -> String {
    guard let userDefaults = UserDefaults(suiteName: "group.com.bitilwallet.bitilwallet"),
