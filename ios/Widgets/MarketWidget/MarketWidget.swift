...
struct MarketWidgetProvider: TimelineProvider {
  func placeholder(in context: Context) -> MarketWidgetEntry {
    return MarketWidgetEntry(date: Date(), marketData: MarketData(nextBlock: "15", sats: "3.003", price: "$0.333", rate: 0.333))
  }
  
  func getSnapshot(in context: Context, completion: @escaping (MarketWidgetEntry) -> ()) {
    let entry: MarketWidgetEntry
    if (context.isPreview) {
      entry = MarketWidgetEntry(date: Date(), marketData: MarketData(nextBlock: "15", sats: "3.003", price: "$0.333", rate: 0.333))
    } else {
  ...
  func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
    ...
    if context.isPreview {
      let entry = MarketWidgetEntry(date: Date(), marketData: MarketData(nextBlock: "15", sats: "3.003", price: "$0.333", rate: 0.333))
...
struct MarketWidget_Previews: PreviewProvider {
  static var previews: some View {
    MarketWidgetEntryView(entry: MarketWidgetEntry(date: Date(), marketData: MarketData(nextBlock: "15", sats: "3.003", price: "$0.333", rate: 0.333)))
