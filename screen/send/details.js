...
const SendDetails = () => {
  ...
  const processAddressData = data => {
    ...
    const dataWithoutSchema = data.replace('bitcoinil:', '').replace('BITCOINIL:', '');
    ...
    let options;
    try {
      if (!data.toLowerCase().startsWith('bitcoinil:')) data = `bitcoinil:${data}`;
    ...
    if (btcAddressRx.test(address) || address.startsWith('il1') || address.startsWith('IL1')) {
      setAddresses(addresses => {
