...
const openSignedTransaction = async function () {
  try {
    const res = await DocumentPicker.pick({
      type: Platform.OS === 'ios' ? ['com.bitilwallet.psbt', 'com.bitilwallet.psbt.txn'] : [DocumentPicker.types.allFiles],
    });
  ...
  try {
    const res = await DocumentPicker.pick({
      type:
        Platform.OS === 'ios'
          ? [
              'com.bitilwallet.psbt',
              'com.bitilwallet.psbt.txn',
              'com.bitilwallet.backup',
